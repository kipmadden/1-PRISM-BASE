...
entry to `working_dir/escalations.md` and exits without advancing state. The
...
`scripts/verify_handoff.py` walks the working directory and exits non-zero
if any artifact is missing frontmatter, missing a sidecar, or missing a
required field. The orchestrator runs this check before every phase advance.
...
(`.claude-plugin/plugin.json`). Choice of Claude Code — rather than, for
example, a standalone Python pipeline — is motivated by three considerations:
...
3. The program benefits from heterogeneous model assignments: the
   ...
   Librarian and Data Fetcher to Haiku-class. See `agents/*.md` for
...
Python scripts under `scripts/` and invoked through `Bash`. They do not
require MCP servers beyond the filesystem.
...
├── scripts/                            Python helpers (non-agentic components)
...
1. **`prism_research_plan_v1.0.md`** (program root) — the authoritative
   specification of the research program. Defines all seventeen roles, each
   task, each artifact contract, and the kill criteria.
2. **`working_dir/kip_madden_prism_notes.md`** — principal-investigator
   framework notes. First-person exposition of the core intuitions, the
   history of prior iterations (v1 through v4), and direct responses to the
   seven open questions raised by the Theoretician at P1-T1.
3. **`working_dir/derivations/prism_formal_spec.md`** — the Theoretician's
   formal translation of the postulate set into symbolic form, including the
   5D→4D coordinate transformation and the Jacobian derivation.
...
`templates/knowledge_base/literature_values.yaml`.
...
documentation and are specified per-agent in `agents/*.md`.
